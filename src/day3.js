...
const lines = fs
  .readFileSync("src/day3.txt", { encoding: "utf-8" })
  .split("\n")
  .filter((x) => Boolean(x));
...
function getOxygen(lines, index = 0) {
  const { zeros, ones } = getCount(lines);
  let mostCommonBit = "1";
...
function part2() {
  const oxygenRating = getOxygen(lines);
  const CO2Rating = getCO2(lines);
